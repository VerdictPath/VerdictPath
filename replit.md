...
**Recent Implementations (November 10-11, 2025):**
- **Comprehensive Beta Test Accounts (November 11, 2025)**: Created production-ready test accounts for beta testing via `backend/scripts/seed-beta-accounts.js`. Includes 3 fully-featured accounts: Individual User (beta_individual) with 44% roadmap completion, 2 tasks, 1 appointment, and links to 4 medical providers; Law Firm (beta_lawfirm) with 42 clients, 10 active bill negotiations at various stages, and premium plan access; Medical Provider (beta_provider) with 23 patients, 15 notifications (10 unread), and premium plan access. All accounts use password: password123. Additional data: 42 client accounts (mixed tiers), 23 patient accounts, 4 medical provider accounts, 18 litigation substage completions, and comprehensive relational data. User records created for all medical providers to enable proper client-provider linking via users table foreign keys. Full documentation in docs/BETA_TEST_ACCOUNTS.md.
- **Bill Negotiations Portal**: Full-featured negotiation system allowing law firms and medical providers to negotiate medical bills directly in-app. Features include negotiation initiation with client/patient and provider selection, counter-offer workflow, offer acceptance, phone call request mechanism, and downloadable negotiation logs. Backend implements negotiations and negotiation_history tables with full audit trail. Both law firms and medical providers can initiate and manage negotiations through a unified interface with role-specific workflows. Enhanced UI includes auto-selection of medical providers, warning boxes for missing relationships, "Your Turn" badges, comprehensive negotiation history modals, and role-specific counter-offer validation (law firms negotiate DOWN to pay less, medical providers negotiate UP to get paid more). **Quick Actions section** added to both Law Firm and Medical Provider main dashboard tabs, making Bill Negotiations immediately accessible without tab switching.
- **Client-Medical Provider Relationship Management**: Secure system for tracking which medical providers a client/patient has visited. Law firms control provider linking via dedicated API endpoints (POST /link-medical-provider, GET /clients/:clientId/medical-providers, DELETE for removal). Database implements client_medical_providers table with soft deletes and primary provider designation. Authorization enforces law-firm-only relationship management to ensure consent and prevent PHI exposure. Integration with negotiations portal ensures law firms see only authorized providers when initiating bill negotiations.
...
- **Subscription Screen Navigation Fix (November 11, 2025)**: Fixed blank screen issue when law firms and medical providers attempted to upgrade to premium from the disbursement dashboard. Added proper screen registration for LawFirmSubscriptionScreen and MedicalProviderSubscriptionScreen in App.js. Implemented back buttons on both subscription screens for improved UX, allowing users to return to their dashboards. This ensures users can now successfully navigate to subscription management screens and upgrade their plans without encountering blank screens.
- **Contact Email Standardization (November 11, 2025)**: Updated all contact email addresses throughout the app from parsonslawfirm@gmail.com and support@verdictpath.com to contact@verdictpath.io. Changes applied to mobile app screens (Privacy Policy, Terms of Service, EULA), legal text assets, and README documentation (8 files, 20 instances).
- **New Nautical Pirate Logo (November 11, 2025)**: Replaced app logo system-wide with new nautical pirate logo featuring foggy sea background and treasure map design. Updated LandingScreen.js, app.json (app icon, splash screen, Android adaptive icon), and web bundle. Removed legacy logo files (LOGONEW_1762465474031.png, verdict-path-logo.png). Logo path: `attached_assets/Nautical Pirate Logo with Foggy Sea Background_1762830868803.png`. Splash background updated to #2C5F7C to complement nautical theme.
...
- **docs/BETA_TEST_ACCOUNTS.md**: Comprehensive documentation of beta test accounts with login credentials, feature specifications, data statistics, testing workflows, and cleanup scripts. Includes 3 main test accounts (individual, law firm, medical provider) with extensive supporting data (42 clients, 23 patients, 10 negotiations, 15 notifications).
